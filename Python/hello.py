...
def main(name):
    ...
    def choice3():
        ...
        bg_color = "#61B63D"
        choice_win.configure(bg=bg_color)
        canvas = Canvas(choice_win, width=510, height=300, bg=bg_color, highlightthickness=0)
        canvas.pack()
        rules_text = ("The rules of this game are:\n"
                    "The plane body is blue, the nose is red, and the rest is white.\n"
                    "The winner is the one who finds the noses of both planes.\n"
                    "The planes look like this:")
        canvas.create_text(255, 70, text=rules_text, font=("Ariel", 14), fill="black", width=480)
        rule_img = PhotoImage(file="a.png")
        rule_small = rule_img.subsample(4,4)
        choice_win._rule_img = rule_small
        canvas.create_image(160,200, image=rule_small)
        play_btn = Button(choice_win, text="Play", command=lambda: (choice_win.destroy(), click3()), bg="white", borderwidth=0)
        canvas.create_window(320,200, window=play_btn, width=120, height=40)
    ...
    def click3():
        ...
        if best_steps==101:
            best=canvasplane.create_text(835,200,text=f"Best Steps:{0}", font=("Ariel",25))
        else:
            best=canvasplane.create_text(835,200,text=f"Best Steps:{best_steps}", font=("Ariel",25))
        def increasesteps():
            nonlocal zero
            zero+=1
        ...
        def playagain():
            ...
            yey= Toplevel()
            yey.title("game over")
            labelyey= Label(yey, text="You win!\n Your best steps is "+str(best_steps))
        ...
        def plane4():
            def stopplaness():
                ...
                s98 = buttons83.cget("state")
                if s8 == DISABLED and s98 == DISABLED:
                    playagain()
            def plane41red():
            ...
            def plane42red():
                buttons98.config(bg="red")
                increasesteps()
                buttons98.config(state=DISABLED)
                stopplaness()
            buttons98.config(command=plane42red)
            def plane42blue1():
                buttons67.config(bg="blue")
                increasesteps()
                buttons67.config(state=DISABLED)
    ...
    def choice4():
        ...
        choice_win.configure(bg=bg_color)
        rules_text = ("The rules of this game are:\n"
                    "I'll scramble the words,\nand you need to write the original word.\n"
                    "(If you don't know the answer,\njust click submit to find out. :)")
        lbl = Label(choice_win, text=rules_text, font=("Ariel", 16), bg=bg_color, fg="black")
        lbl.place(x=20, y=40)
        play_btn = Button(choice_win, text="Play", command=lambda: (choice_win.destroy(), click4()),
                        bg="white", borderwidth=0, highlightthickness=0)
        play_btn.place(x=210,y=170,width=60, height=30)
    def click4():
        game4 = Toplevel()
        game4.title("Anagram")
        back = PhotoImage(file="normal.png")
    ...
    button5= Button(window,text=name,borderwidth=0, highlightthickness=0,font=("Arial",16))
    button5.place(x=200,y=30,width=120,height=40)
    label.pack()
    button1.config(command=choice1)
    button2.config(command=choice2)
    button3.config(command=choice3)
    button4.config(command=choice4)
    window.mainloop()
